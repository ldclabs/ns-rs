...
COPY . .
RUN xx-cargo build --release --package ns-indexer --bin ns-indexer \
...
WORKDIR /app
COPY --from=builder /src/crates/ns-indexer/sample.env ./.env
COPY --from=builder /src/release/ns-indexer ./ns-indexer

ENTRYPOINT ["./ns-indexer"]
